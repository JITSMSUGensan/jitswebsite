...
const fadefadeNavigationBackdrop = (navigationBackdrop, scroller) => {
    ...
    if(scrollTop > 1) {
        if(!navigationBackdrop.classList.contains("faded-in")) {
            animateElement(navigationBackdrop, animation.keyframe.fadeIn85, animation.options.durationXMsAndPermanent(350));
            navigationBackdrop.classList.add("faded-in");
        } 
    } else {
        if(navigationBackdrop.classList.contains("faded-in")) {
            animateElement(navigationBackdrop, animation.keyframe.fadeOut85, animation.options.durationXMsAndPermanent(350));
